...
+ (UIImage*) GetFirstImageFromRecordFile: (NSString*) strRecPath
{
    ...
    if (pfile == NULL) {
        NSLog(@"GetFirstImageFromRecordFile..打开文件失败");
        return nil;
    }
    ...
    if (sizeof(filehead) != fread((char*)&filehead, 1, sizeof(filehead), pfile)) {
        fclose(pfile);
        pfile = NULL;
        NSLog(@"GetFirstImageFromRecordFile..读取头文件失败");
        return nil;
    }
    
    if (filehead.head != 0xff00ff00) {
        NSLog(@"GetFirstImageFromRecordFile..文件头不是0xff00ff00");
        fclose(pfile);
        pfile = NULL;
        return nil;
    }
    ...
    if (sizeof(datahead) != fread((char*)&datahead, 1, sizeof(datahead), pfile)) {
        fclose(pfile);
        pfile = NULL;
        NSLog(@"GetFirstImageFromRecordFile..读取录像文件第一帧头部失败");
        return nil;
    }
    ...
    if (datahead.head != 0xffff0000) {
        fclose(pfile);
        pfile = NULL;
         NSLog(@"GetFirstImageFromRecordFile..读取录像文件第一帧头部不是0xffff0000");
        return nil;
    }
    ...
    if (filehead.videoformat == 3) {//MJPEG
