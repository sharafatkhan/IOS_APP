...
+ (void) showWithText: (NSString*) strText superView: (UIView*) superView bLandScap:(BOOL) bLandScap
{
    ...
    if (mainS.size.width<mainS.size.height) {
        CGFloat h=mainS.size.height;
        mainS.size.height=mainS.size.width;
        mainS.size.width=h;
    }
    screenWidth=mainS.size.width;
    screenHeight=mainS.size.height;
