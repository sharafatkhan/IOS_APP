...
{ 
    ...
    m_SetLock=[[NSCondition alloc]init];
    m_AlarmLogsLock=[[NSCondition alloc]init];
...
void CPPPPChannel::SetServer(NSString *server){
    strServer=server;
}
...
{  
    ...
    m_BufRecordAVIVideo=new CCircleBuf();
    m_BufRecordAVIAudio=new CCircleBuf();
...
void CPPPPChannel::Stop()
{
    ...
    if (m_bAVIRecordThreadRunning==1) {
        stopRecordAVI();
    }
    ...
    SAFE_DELETE(m_BufRecordAVIVideo);
    SAFE_DELETE(m_BufRecordAVIAudio);
    SAFE_DELETE(aviRecordMgt);
...
int CPPPPChannel::AddCommand(void * data, int len)
{
    ...
    if (m_pCommandBuffer!=NULL) {
        //NSLog(@"AddCommand....m_pCommandBuffer!=NULL");
...
void CPPPPChannel::ProcessWifiScanResult(STRU_WIFI_SEARCH_RESULT_LIST wifiSearchResultList)
{
    ...
    {
        int bEnd = 0;
        wifiSearchResultList.wifi[i].bEnd=0;
        if (i == nCount - 1) 
        {
            bEnd = 1;
            wifiSearchResultList.wifi[i].bEnd=1;
            
        }
        if (m_WifiParamsDelegate!=nil) {

            
            [m_WifiParamsDelegate  WifiScanProtocolResult:wifiSearchResultList.wifi[i]];
        }
...
{
   // NSLog(@"ProcessWifiParam.....");
    [m_WifiParamsLock lock];
    if (m_WifiParamsDelegate != nil) {
   
        [m_WifiParamsDelegate WifiProtocolResult:wifiParams];
    }
    [m_WifiParamsLock unlock];
}
void CPPPPChannel::ProcessUserInfo(STRU_USER_INFO userInfo)
{
    ...
    if (m_UserPwdParamsDelegate != nil) {
        ...
        [m_UserPwdParamsDelegate UserProtocolResult:userInfo];
    }
...
void CPPPPChannel::ProcessFtpParams(STRU_FTP_PARAMS ftpParam)
{
    ...
    if (m_FtpDelegate != nil) {
        ...
        [m_FtpDelegate FtpProtocolResult:ftpParam];
    }
    [m_FTPLock unlock];
}
...
void CPPPPChannel::ProcessMailParams(STRU_MAIL_PARAMS mailParam)
{
    ...
    if (m_MailDelegate != nil) {
        ...
        [m_MailDelegate MailProtocolResult:mailParam];
    }
    [m_MailLock unlock];
}
...
void CPPPPChannel::ProcessDatetimeParams(STRU_DATETIME_PARAMS datetime){
    ...
    if (m_DateTimeParamsDelegate != nil) {
        ...
        [m_DateTimeParamsDelegate DateTimeProtocolResult:datetime];
...
{
   // NSLog(@"ProcessAlarmParams...111111");
    [m_AlarmParamsLock lock];
    if (m_AlarmParamsDelegate != nil) {

        [m_AlarmParamsDelegate AlarmProtocolResult:alarm];
    }
...
{
    //NSLog(@"Call ProcessCommand cmd: %x, len: %d", cmd, len);
    
    switch(cmd)
    {
        case CGI_TALK_OPEN_CLOSE:
...
//            break;
        ...
        case CGI_IEGET_ALARMLOG:
        {
             ...
             NSLog(@"CGI_IEGET_ALARMLOG pbuf: %s\n", pbuf);
            int ncount = 0;
            SscanfInt(pbuf,"ncount=",ncount);
            if (ncount>30) {
                ncount = 30;
            }
            NSLog(@"CGI_IEGET_ALARMLOG:%d",ncount);
            char buf[128] = {0};
            [m_AlarmLogsLock lock];
            for (int i = 0;i<ncount;i++) {
                sprintf(buf, "year[%d]=",i);
                int year = 0;
                SscanfInt(pbuf, buf, year);
                
                int mon = 0;
                sprintf(buf, "mon[%d]=",i);
                SscanfInt(pbuf, buf, mon);
                
                int day = 0;
                sprintf(buf, "day[%d]=",i);
                SscanfInt(pbuf, buf, day);
                
                int hour = 0;
                sprintf(buf, "hour[%d]=",i);
                SscanfInt(pbuf, buf, hour);
                
                int min = 0;
                sprintf(buf, "min[%d]=",i);
                SscanfInt(pbuf, buf, min);
                
                int sec = 0;
                sprintf(buf, "sec[%d]=",i);
                SscanfInt(pbuf, buf, sec);
                
                int actiontype = 0;
                sprintf(buf, "actiontype[%d]=",i);
                SscanfInt(pbuf, buf, actiontype);
                int bEnd = 0;
                if (i==ncount-1) {
                    bEnd = 1;
                }
                
                NSLog(@"year:%d,mon:%d,day:%d,hour:%d,min:%d,sec:%d,actiontype:%d",year,mon,day,hour,min,sec,actiontype);
                if (m_AlarmLogsDelegate!=nil) {
                    [m_AlarmLogsDelegate CallbackAlarmLoglist:year MON:mon DAY:day HOUR:hour MIN:min SEC:sec ACTIONTYPE:actiontype NCOUT:ncount BEND:bEnd];
                }
            }
            [m_AlarmLogsLock unlock];
        }
            break;
        case CGI_IESET_USER:
        ...
        {
            int nResult=m_CgiPacket.UnpacketSetResult(pbuf);
            NSLog(@"CGI_IESET_WIFI: %s", pbuf);
            ProcessResult(nResult, CGI_IESET_WIFI);
        }
            break;
        case CGI_IESET_ALARM:
         //   ProcessResult(pbuf, len, MSG_TYPE_SET_ALARM);
            break;
        case CGI_HKWS_SDCARD_LIST:
...
int CPPPPChannel::SscanfInt(const char *pszBuffer , const char *reg , int &nResult)
{
...
void CPPPPChannel::SetResultDelegate(id delegate){
    [m_SetLock lock];
    m_SetResultDelegate=delegate;
    [m_SetLock unlock];
}
void CPPPPChannel::ProcessResult(int result, int type){
    if (m_SetResultDelegate!=nil) {
        [m_SetResultDelegate setResult:result];
    }
}
void CPPPPChannel::ProcessCheckUser(char *pbuf, int len)
{
    ...
    //    int nMsg = 0;
    sscanf(p, "result=%d;\r\n", &result);
    NSLog(@"check_user..result=%d",result);
    if (result == 0)
    {
...
//        if ([m_SDcardScheduleDelegate respondsToSelector:@selector(sdcardScheduleParams:Tota:RemainCap:SD_status:Cover:TimeLength:FixedTimeRecord:RecordSize:RecordAudio:)]) {
//            [m_SDcardScheduleDelegate sdcardScheduleParams:[NSString stringWithUTF8String:szDID] Tota:recordParam.sdtotal RemainCap:recordParam.sdfree SD_status:recordParam.record_sd_status Cover:recordParam.record_cover_enable TimeLength:recordParam.record_timer FixedTimeRecord:recordParam.record_time_enable RecordSize:recordParam.record_size RecordAudio:recordParam.enable_record_audio];
//        }
        [m_SDcardScheduleDelegate SDCardProtocolResult:recordParam];
    }
    
   
    [m_SDCardScheduleLock unlock];
}
void CPPPPChannel::SetSDcardOver(int over){
    sdcardOver=over;
}
void CPPPPChannel::ProcessRecordFile(STRU_RECORD_FILE_LIST recordFileList)
{
    NSLog(@"ProcessRecordFile...%d",recordFileList.nPageCount);
    ...
    for(i = 0; i < nCount; i++)
    {
        ...
        if (m_SDCardSearchDelegate!=nil) {
            [m_SDCardSearchDelegate SDCardRecordFileSearchResult:[NSString stringWithFormat:@"%s", recordFileList.recordFile[i].szFileName] fileSize:recordFileList.recordFile[i].nFileSize bEnd:bEnd PageCount:nPageCount RecordBend:recordFileList.recordFile[i].record_bend];
        }
    ...
    if (nCount==0) {
        ...
        if (m_SDCardSearchDelegate!=nil) {
            [m_SDCardSearchDelegate SDCardRecordFileSearchResult:@"" fileSize:0 bEnd:bEnd PageCount:nPageCount RecordBend:recordFileList.recordFile[i].record_bend];
        }
    }
...
{
    NSLog(@"ProcessCameraParams....pbuf=%s",pbuf);
//    NSLog(@"pbuf: %s", pbuf);
//    NSLog(@"len: %d", len);
//    
//    resolution=0;
...
int CPPPPChannel::PTZ_Control(int command)
{
    ...
    switch (command) {
        case CMD_PTZ_UP_RUN:
            ...
            onestep=0;
            break;
        case CMD_PTZ_DOWN_RUN:
            command=CMD_PTZ_DOWN;
            onestep=0;
            break;
        case CMD_PTZ_LEFT_RUN:
            command=CMD_PTZ_LEFT;
            onestep=0;
            break;
        case CMD_PTZ_RIGHT_RUN:
   ...
   sprintf(buf, "GET /decoder_control.cgi?command=%d&onestep=%d&loginuse=%s&loginpas=%s&user=%s&pwd=%s&", command, onestep, szUser, szPwd, szUser, szPwd);
    NSLog(@"云台控制 cgi:%s",buf);
...
#pragma mark---Record AVI Video start
void CPPPPChannel::startRecordAVI(char *path, char *format,int width,int height){
    //NSLog(@"startRecordAVI.....path=%s",path);
    m_BufRecordAVIVideo->Create(VBUF_SIZE);
    m_BufRecordAVIAudio->Create(VBUF_SIZE);
    aviRecordMgt->startAvi(szDID, path, width, height, 15, format);
    m_bAVIRecordThreadRunning=1;
    pthread_create(&m_AVIThreadID, NULL, startRecordAVIChannel, (void*)this);
}
void* CPPPPChannel::startRecordAVIChannel(void *param){
    CPPPPChannel *p=(CPPPPChannel*)param;
    p->startRecordAVIProcess();
    return NULL;
}

void CPPPPChannel::startRecordAVIProcess(){
    while (m_bAVIRecordThreadRunning) {
        
        if (m_BufRecordAVIVideo->GetStock()!=0) {
            char *pbuf = NULL;
            int videoLen = 0;
            ...
            if (pbuf!=NULL) {
                //NSLog(@"avi 写一帧视频数据");
                aviRecordMgt->writeVideoData(szDID, pbuf, videoLen, videohead.frametype);
            }else{
                //NSLog(@"avi 写一帧空视频数据");
                char buf[0];
                memset(buf, 0, sizeof(buf));
                aviRecordMgt->writeVideoData(szDID, buf, sizeof(buf), 1);
            }
        }else{
            char buf[1]={0};
            memset(buf, 0, sizeof(buf));
            //NSLog(@"avi 写一帧空视频数据");
            aviRecordMgt->writeVideoData(szDID, buf, 0, 0);
        }
        for (int i=0; i<3; i++) {
            
            char bufAudio[2048]={0};
            if(0!=m_BufRecordAVIAudio->Read(bufAudio,2048)){
                // NSLog(@"avi 写一帧音频数据");
                aviRecordMgt->writeAudioData(szDID, bufAudio, 2048);
            }else{
                // NSLog(@"avi 写一帧空音频数据");
                char buf[0];
                memset(buf, 0, sizeof(buf));
                aviRecordMgt->writeAudioData(szDID, buf, sizeof(buf));
            }
        }
        
        usleep(67000);
    }
    NSLog(@"startRecordAVIProcess....end");
}
void CPPPPChannel::stopRecordAVI(){
    
    m_bAVIRecordThreadRunning=0;
    NSLog(@"stopRecordAVI....");
    pthread_join(m_AVIThreadID, NULL);
    
    aviRecordMgt->closeAvi(szDID);
    m_BufRecordAVIVideo->Release();
    m_BufRecordAVIAudio->Release();
...
//			avhead.type, avhead.streamid, avhead.militime, avhead.sectime, avhead.len, avhead.frameno);
        ...
        if (m_bAVIRecordThreadRunning==1) {
            if (m_BufRecordAVIAudio!=NULL) {
                m_BufRecordAVIAudio->Write(pPCMBuf, nPCMLen);
            }
            
        }
        [m_PlayViewAVDataDelegateLock lock];
        if (m_PlayViewImageNotifyDelegate!=nil) {
            [m_PlayViewImageNotifyDelegate AudioDataBack:(unsigned char*)pPCMBuf Length:nPCMLen];
        }
...
int CPPPPChannel::StartTalk()
{
    ...
    cgi_get_common((char*)"talkstream.cgi?streamid=1&");
...
{  
    ...
    cgi_get_common((char*)"talkstream.cgi?streamid=16&");
...
RE_CONNECT:
    ...
    {        
        ...
        NSLog(@"PPPP_Connect begin...");
        MsgNotify(MSG_NOTIFY_TYPE_PPPP_STATUS, PPPP_STATUS_CONNECTING);
        NSLog(@"========strServer=%@",strServer);
        m_hSessionHandle=PPPP_ConnectByServer(szDID, 1, 0, (char*)[strServer UTF8String]);
       // m_hSessionHandle = PPPP_Connect(szDID, 1, 0);
         NSLog(@"PPPPChannel---CommandProcess---m_hSessionHandle=%d",m_hSessionHandle);
        if(m_hSessionHandle < 0)
        {
...
//    StartAlarmChannel();
    ...
    {
        //NSLog(@"m_bDataThreadRuning[%d]",m_bDataThreadRuning);
        UINT32 uiReadSize = 0;
        ...
        int nRet=0;
        if (m_pCommandBuffer!=NULL) {
            nRet = m_pCommandBuffer->Read((char*)&bufhead, nHeadLen);
        }
        
        if(nRet == 0)
        {
           // NSLog(@"the CommandBuffer is empty...");
            usleep(10000);
            continue;
        }
        ...
        if(res < 0)
        {
           NSLog(@"CommandProcess PPPP_Write PPPP_Write return failed: %d", res);
            if (res != ERROR_PPPP_SESSION_CLOSED_CALLED) {
                NSLog(@"CommandProcess PPPP_Write PPPP_Write return failed: %d", res);
                PPPPClose();
            }
...
{    
    while(m_bDataThreadRuning)
    {
        ...
        memset(&avhead, 0, sizeof(avhead));
        //NSLog(@"开始读实时视频数据的头文件");
        //NSLog(@"DataProcess PPPP_IndeedRead-start[%d]",m_bDataThreadRuning);
        int nRet = PPPP_IndeedRead(P2P_VIDEOCHANNEL, (CHAR*)&avhead, sizeof(avhead));
        // NSLog(@"DataProcess PPPP_IndeedRead-end[%d]",nRet);
        if(nRet < 0)
        {
            NSLog(@"DataProcess 读取头部 PPPP_IndeedRead failed  return: %d", nRet);
            PPPPClose();
            return ;
        }
       ...
       // NSLog(@"avhead.frameno=%d type=%d",avhead.frameno,avhead.type);
        //NSLog(@"开始读实时视频数据");
...
//        if (m_PlayViewImageNotifyDelegate!=nil) {
//            [m_PlayViewImageNotifyDelegate readPPPPChannelDataSpeed];
//        }
        
        int streamType;
        if ( avhead.type == 0 || avhead.type == 1) {
        ...
        if(m_EnumVideoMode == ENUM_VIDEO_MODE_H264)
        {
                ...
                if(avhead.type == ENUM_FRAME_TYPE_P || avhead.type == ENUM_FRAME_TYPE_I)
                {
                    ...
                    {//i帧来了
                        //NSLog(@"实时视频i来了，清掉buf");
                         m_pVideoBuf->Reset();
                        if (m_pVideoBuf->Write(pbuf, Length)) {
                            m_bFindIFrame=true;//i帧保存成功
                            //NSLog(@"实时视频i保存成功");
                        ...
                        if (m_bAVIRecordThreadRunning==1) {
                            m_BufRecordAVIVideo->Reset();
                            m_BufRecordAVIVideo->Write(pbuf, Length);
                    ...
                    else
                    {
                        if (m_bFindIFrame) {
                            ...
                            if (m_bAVIRecordThreadRunning==1) {
                                if (m_BufRecordAVIVideo->Write(pbuf, Length)) {
                                    m_bFindIFrame = false;
                                }
                            }
                        }
...
#pragma mark-----JPush Param Start
void CPPPPChannel::setJPushParam(STRUCT_JPUSH_PARAM jpt){
    
    //jpush_register?appkey=%s&master=%s&alias=%s&type=%d&
    // jpush_delete?appkey=%s&alias=%s&
    char buf[1024];
    sprintf(buf,"jpush_register.cgi?appkey=%s&master=%s&alias=%s&type=%d&",jpt.appkey,jpt.master,jpt.alias,jpt.type);
    cgi_get_common(buf);
}
void CPPPPChannel::deleJPushParam(STRUCT_JPUSH_PARAM jpt){
    char buf[1024];
    sprintf(buf,"jpush_delete.cgi?appkey=%s&alias=%s&",jpt.appkey,jpt.alias);
    cgi_get_common(buf);
}
#pragma mark-----JPush Param End
void CPPPPChannel::PlaybackProcess()
{
   ...
   // const int maxbuffersize = 600 * 1024 ;
    ...
    {
        if (m_bPlaybackStarted ==NO) {
           
            usleep(10000);
            continue;
        }
       
       int curSize= m_pPlayBackVideoBuf->GetStock();
        if (curSize>=VBUF_SIZE) {
            usleep(10000);
            continue;
        }
        //read head
        ...
        if (avhead.type==6) {//音频数据
            NSLog(@"audio...frameno=%d",avhead.frameno);
            // NSLog(@"PPPPChannel.....PlaybackProcess()....收到音频数据");
            int Length=avhead.len;
            char *pbuf=new char[Length+1];
            // NSLog(@"PPPPChannel....PlaybackProcess()....从回放通道中读音频数据.....");
            nRet=PPPP_IndeedRead(P2P_PLAYBACK, (CHAR*)pbuf, Length);
            if (nRet<0) {
                NSLog(@"PlaybackProcess audio error: %d", nRet);
                SAFE_DELETE(pbuf);
                continue;
            }
            //NSLog(@"PPPPPChannel...PlaybackProcess()...mbAudioStarted=%d",m_bAudioStarted);
            if(m_bAudioStarted == 0)
            {
                SAFE_DELETE(pbuf);
                continue;
            }
            // NSLog(@"PPPPChannel....PlaybackProcess()....把读到的 音频数据 放入buf中.....");
            //----- adpcm --> pcm ---------------------------------------
            int nPCMLen = 4 * Length;
            char *pPCMBuf = new char[nPCMLen];
            if (m_pAudioAdpcm!=NULL) {
            ...
            SAFE_DELETE(pbuf);
             ...
             NSLog(@"avhead.nowframe=%d allframe=%d",avhead.nowframe,avhead.allframe);
            int Length = sizeof(VIDEO_BUF_HEAD) + avhead.len;
            ...
            phead->head = VIDEO_HEAD_VALUE;
            phead->timestamp = avhead.sectime* 1000 + avhead.militime;
            phead->sectime=avhead.sectime;
            phead->len = avhead.len;
            phead->frametype = avhead.type ;
            //NSLog(@"PlaybackProcess()...sectime:%d",avhead.sectime);
            ...
            if(m_EnumPlayBackVideoMode == ENUM_VIDEO_MODE_H264)
            {
                    ...
                    if(avhead.type == ENUM_FRAME_TYPE_P || avhead.type == ENUM_FRAME_TYPE_I)
                    {
                        if(avhead.type == ENUM_FRAME_TYPE_I)
                        {
                            ...
                            while (0==m_pPlayBackVideoBuf->Write(pbuf, Length)) {
                                usleep(2000);
                                NSLog(@"保存I帧失败");
                            }
...
void CPPPPChannel::TalkProcess()
{
    ...
    while(m_bTalkThreadRuning)
    {
        ...
        int nRead =MIN_PCM_AUDIO_SIZE;
        if (m_pTalkAudioBuf!=NULL) {
            nRead = m_pTalkAudioBuf->Read((char*)AudioBuf, MIN_PCM_AUDIO_SIZE);
        }
...
void CPPPPChannel::PlaybackVideoPlayerProcess()
{
    ...
    while(m_bPlaybackVideoPlayerThreadRuning)
    {
        ...
        unsigned int untimestamp = videobufhead.sectime;
...
void CPPPPChannel::SetPlayViewParamNotifyDelegate(id delegate)
{
    [m_PlayViewParamNotifyDelegateLock lock];
...
void CPPPPChannel::SetPlayViewPPPPStatusDelegate(id delegate)
{
    [m_PlayViewPPPPStatusDelegateLock lock];    
...
void CPPPPChannel::PlayProcess()
{
    ...
    {
//        if (m_EnumVideoMode == ENUM_VIDEO_MODE_UNKNOWN) {
//             NSLog(@"m_EnumVideoMode == ENUM_VIDEO_MODE_UNKNOWN");
//            usleep(100000);
//            continue;
//        }
...
void CPPPPChannel::ImageData(unsigned char *buf, int len,unsigned int timestamp){
   [m_PlayViewAVDataDelegateLock lock];
    if (m_PlayViewImageNotifyDelegate!=nil) {
        [m_PlayViewImageNotifyDelegate ImageData:buf Length:len timestamp:timestamp];
    }
   [m_PlayViewAVDataDelegateLock unlock];
}
...
void CPPPPChannel::SetPlayViewImageNotifyDelegate(id delegate)
{
...
int CPPPPChannel::SetSystemParams(int type,char * msg,int len)
{
    ...
    switch (type)
    {
            ...
            //nRet = SendDatetimeSetting(msg, len);
        {
            ...
            if (pTemp->now == 0)
            {
                sprintf(buf,"set_datetime.cgi?tz=%d&ntp_enable=%d&ntp_svr=%s&xia_ling_shi_flag_status=%d&",
                          pTemp->tz,pTemp->ntp_enable,pTemp->ntp_svr,pTemp->xia_ling_shi_flag_status
                          );
            }
            else{
               // NSLog(@"PPPChannenl local  now=%d tz=%d ntp_enable=%d ",pTemp->now,pTemp->tz,pTemp->ntp_enable);
                sprintf(buf,"set_datetime.cgi?now=%d&tz=%d&ntp_enable=%d&ntp_svr=%s&xia_ling_shi_flag_status=%d&",
                          pTemp->now,pTemp->tz,pTemp->ntp_enable,pTemp->ntp_svr,pTemp->xia_ling_shi_flag_status
                          );
            }
           ...
           
        {
            ...
            break;
        case MSG_TYPE_FORMAT_SDCARD:
        {
            nRet = cgi_get_common((char*)"set_formatsd.cgi?");
        }
            break;
        ...
        case MSG_TYPE_SET_PTZ:
            ...
            break;
        case MSG_TYPE_PLAYALARM:
        {
            char buf[1024];
            STRU_ALARM_PARAMS *pTemp = (STRU_ALARM_PARAMS *)msg;
            sprintf(buf, "set_alarm.cgi?motion_armed=%d&input_armed=%d&",pTemp->motion_armed,pTemp->input_armed);
             nRet = cgi_get_common(buf);
        }
            break;
            ...
            
        {
            ...
            sprintf(buf, "get_record_file.cgi?PageIndex=%d&PageSize=%d&PageTime=%d&",pRecordSearch->pageIndex, pRecordSearch->pageSize,pRecordSearch->pagetime);
            nRet = cgi_get_common(buf);
        ...
        case MSG_TYPE_GET_RECORD:
            ...
            break;
        case MSG_TYPE_GET_ALERM_LOG:
        {
         nRet = cgi_get_common((char*)"get_sys_alarm_log.cgi?&start_index=0&read_nums=20&");
        }
            break;
        default:
...
int CPPPPChannel::SetDateTime(int now,int tz,int ntp_enable,char *ntp_svr,int xialishi)
{
    ...
    datetimePama.xia_ling_shi_flag_status = xialishi;
...
void CPPPPChannel::SetAlarmLogsDelegate(id delegate){
    [m_AlarmLogsLock lock];
    m_AlarmLogsDelegate=delegate;
    [m_AlarmLogsLock unlock];
}

int CPPPPChannel::SetPlayAlarm(int motion_armed,int input_armed){
    ...
    return SetSystemParams(MSG_TYPE_PLAYALARM, (char*)&alarmParam, sizeof(alarmParam));
}
             ...
             int enable_alarm_audio)
{
    ...
    alarmParam.enable_alarm_audio=enable_alarm_audio;
...
int CPPPPChannel::SetSDCardScheduleParams(
    ...
    recordParam.enable_record_audio=enable_record_audio;
