...
- (BOOL) CreateTable:(NSString *)tbName
{
    ...
    char createTableSql[256];
    memset(createTableSql, 0, sizeof(createTableSql));
    sprintf(createTableSql, "CREATE TABLE IF NOT EXISTS %s(ID INTEGER PRIMARY KEY AUTOINCREMENT,name text, did text, user text, pwd text,dele text)",
            [tbName UTF8String]);
    sqlite3_stmt *statement;
    NSInteger SqlOK = sqlite3_prepare(db,
                                      createTableSql,
...
- (BOOL)InsertCamera:(NSString *)name DID:(NSString *)did User:(NSString *)user Pwd:(NSString *)pwd Dele:(NSString *)dele
{
    sqlite3_stmt *statement;
    char insertSql[256];
    memset(insertSql, 0, sizeof(insertSql));
    sprintf(insertSql, "INSERT INTO %s(name,did,user,pwd,dele) VALUES(?,?,?,?,?)",[TableName UTF8String]);
    ...
    sqlite3_bind_text(statement, 5, [dele UTF8String], -1, nil);
...
- (BOOL) UpdateCameraDeleteFromLocal:(NSString*)did{
    char updateSql[256];
    memset(updateSql, 0, sizeof(updateSql));
    sprintf(updateSql, "UPDATE %s SET dele=? WHERE did=?",[TableName UTF8String]);
    sqlite3_stmt *statement;
    int preOk=sqlite3_prepare(db, updateSql, -1, &statement, nil);
    if (preOk==SQLITE_OK) {
        
    }else{
        NSLog(@"UpdateCameraDeleteFromLocal failed!");
        return NO;
    }
    sqlite3_bind_text(statement, 1, [@"1" UTF8String], -1, SQLITE_TRANSIENT);
    sqlite3_bind_text(statement, 2, [did UTF8String], -1, SQLITE_TRANSIENT);
    int sqlOK = sqlite3_step(statement);
    sqlite3_finalize(statement);
    if (sqlOK == SQLITE_DONE) {
        NSLog(@"UpdateCameraDeleteFromLocal success!");
    }else {
        NSLog(@"UpdateCameraDeleteFromLocal failed!");
        return NO;
    }
...
- (void) SelectAll
{
    ...
    while(sqlite3_step(statement)== SQLITE_ROW)
    {
        ...
        char *str_dele=(char*)sqlite3_column_text(statement, 5);
        ...
        NSString *nsUser = [NSString stringWithUTF8String:str_user];
        NSString *nsPwd = [NSString stringWithUTF8String:str_pwd];
         NSString *nsDele = [NSString stringWithUTF8String:str_dele];
        //        NSString *nsIp=[NSString stringWithUTF8String:str_ip];
        //        NSString *nsPort=[NSString stringWithUTF8String:str_port];
        
        [self.selectDelegate SelectP2PResult:nsName DID:nsDID User:nsUser Pwd:nsPwd Dele:nsDele];
